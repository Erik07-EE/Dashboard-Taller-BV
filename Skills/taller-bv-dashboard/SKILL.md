...
description: >-
  Actualizar el Dashboard de Producción del Taller BV (Electroestrada). Usar
  cuando el usuario diga "actualizar dashboard", "actualizar producción", "sync
  dashboard", "sincronizar pedidos" o variantes. Descarga el Google Sheet como
  XLSX, extrae producción y pedidos, muestra un PREVIEW, aplica cambios al HTML,
  actualiza el badge y valida. Complementaria: taller-bv-dashboard-tech (código).
---
...
- **Fila 1:** objetivos. Buscar celdas "Objetivo Julio"/"Objetivo Junio" y "Obj. S.1"…"Obj. S.5". El valor está en `col+1`.
...
4. Extraer producción + pedidos del XLSX con openpyxl (ver skill técnica).
...
- **% prom.:** `avg((dias_prod - dias_objetivo) / dias_objetivo)` para pedidos con `dias_prod != null`.
- **% Servicio:** `avg(demora)` solo de pedidos con `demora != null` (entregados reales = columna Q del sheet).
